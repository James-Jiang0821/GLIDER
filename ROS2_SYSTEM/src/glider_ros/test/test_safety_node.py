...
Tests are split into two classes:
    TestSafetyNodeInternal: Internal, no ROS topics involved
    TestSafetyNodePubSub: External, verifies that the node publishes expected messages
"""
import threading
import time
...
class TestSafetyNodeInternal:
    ...
    def test_proximity_below_threshold_latches_after_consecutive_reads(self, safety_node):
        msg = Float64Stamped()
        msg.data = 2.0  #default threshold is 5.0 m
        required = safety_node._proximity_consecutive_required

        #Below-threshold reads up to required - 1 must not latch
        for _ in range(required - 1):
            safety_node._cb_range(msg)
            assert safety_node._emergency_latched is False

        #The N-th consecutive reading latches
        safety_node._cb_range(msg)
        assert safety_node._emergency_latched is True
        assert 'Proximity' in safety_node._emergency_detail

    def test_proximity_single_below_threshold_does_not_latch(self, safety_node):
        msg = Float64Stamped()
        msg.data = 2.0
        safety_node._cb_range(msg)
        assert safety_node._emergency_latched is False

    def test_proximity_counter_resets_on_good_reading(self, safety_node):
        bad = Float64Stamped(); bad.data = 2.0
        good = Float64Stamped(); good.data = 10.0
        required = safety_node._proximity_consecutive_required

        #Accumulate required - 1 bad reads, then a good one resets the counter
        for _ in range(required - 1):
            safety_node._cb_range(bad)
        safety_node._cb_range(good)
        assert safety_node._proximity_below_count == 0

        #Another single bad read should not latch
        safety_node._cb_range(bad)
        assert safety_node._emergency_latched is False
