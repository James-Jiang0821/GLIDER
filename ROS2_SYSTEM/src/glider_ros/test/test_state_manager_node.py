...
Tests are split into four classes:
    TestStateManagerCallbacks: Internal, no ROS topics involved
    TestStateManagerStep: Internal, no ROS topics involved
    TestStateManagerEmergencyPhases: Internal, no ROS topics involved
    TestStateManagerPubSub: External, verifies that the node publishes expected messages
"""
import threading
import time
...
class TestStateManagerCallbacks:
    ...
    def test_mission_inject_sets_pending_mission_depth(self, state_node):
        msg = Float64()
        msg.data = 15.5
        state_node._mission_inject_cb(msg)
        assert state_node.pending_mission_depth == pytest.approx(15.5)
    ...
    def test_mission_inject_not_in_idle_is_ignored(self, state_node):
        ...
        assert state_node.pending_mission_depth is None
