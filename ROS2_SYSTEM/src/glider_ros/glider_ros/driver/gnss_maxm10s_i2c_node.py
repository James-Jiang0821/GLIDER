#!/usr/bin/env python3
"""u-blox MAX-M10S I2C driver that parses UBX frames and publishes fixes on /gps/fix."""
import time
import struct
from collections import deque

from smbus import SMBus

import rclpy
from rclpy.node import Node

from sensor_msgs.msg import NavSatFix, NavSatStatus
from geometry_msgs.msg import TwistStamped
from diagnostic_msgs.msg import DiagnosticArray, DiagnosticStatus, KeyValue


UBX_SYNC_1 = 0xB5
UBX_SYNC_2 = 0x62

UBX_CLASS_NAV = 0x01
UBX_ID_NAV_PVT = 0x07  #main PVT solution

#u-blox DDC/I2C registers (MAX-M10S): bytes-available is 16-bit split across 0xFD (MSB) and 0xFE (LSB)
REG_AVAIL_H = 0xFD
REG_AVAIL_L = 0xFE
REG_STREAM = 0xFF


def ubx_checksum(payload: bytes) -> tuple[int, int]:
    ck_a = 0
    ck_b = 0
    for b in payload:
        ck_a = (ck_a + b) & 0xFF
        ck_b = (ck_b + ck_a) & 0xFF
    return ck_a, ck_b


class UbxStreamParser:
    """Minimal UBX framing parser yielding (cls, id, payload_bytes) messages."""
    def __init__(self, max_payload=2048):
        self.buf = deque()
        self.max_payload = max_payload

    def feed(self, data: bytes):
        for b in data:
            self.buf.append(b)

    def _pop(self) -> int:
        return self.buf.popleft()

    def _peek(self, i: int) -> int:
        return self.buf[i]

    def messages(self):
        msgs = []
        while True:
            if len(self.buf) < 8:
                break

            if self._peek(0) != UBX_SYNC_1 or self._peek(1) != UBX_SYNC_2:
                self._pop()
                continue

            length = self._peek(4) | (self._peek(5) << 8)
            if length > self.max_payload:
                self._pop()
                continue

            total_len = 2 + 4 + length + 2
            if len(self.buf) < total_len:
                break

            self._pop()  #sync1
            self._pop()  #sync2
            cls_b = self._pop()
            id_b = self._pop()
            len_l = self._pop()
            len_h = self._pop()
            payload = bytes(self._pop() for _ in range(length))
            ck_a = self._pop()
            ck_b = self._pop()

            ck_payload = bytes([cls_b, id_b, len_l, len_h]) + payload
            exp_a, exp_b = ubx_checksum(ck_payload)
            if (ck_a, ck_b) != (exp_a, exp_b):
                continue

            msgs.append((cls_b, id_b, payload))

        return msgs


def parse_nav_pvt(payload: bytes):
    """Parse UBX-NAV-PVT (92 bytes); returns dict of fields or None if wrong length."""
    if len(payload) < 92:
        return None

    iTOW = struct.unpack_from("<I", payload, 0)[0]
    year = struct.unpack_from("<H", payload, 4)[0]
    month = payload[6]
    day = payload[7]
    hour = payload[8]
    minute = payload[9]
    sec = payload[10]
    fix_type = payload[20]
    flags = payload[21]
    num_sv = payload[23]

    lon = struct.unpack_from("<i", payload, 24)[0] * 1e-7
    lat = struct.unpack_from("<i", payload, 28)[0] * 1e-7
    height_msl_mm = struct.unpack_from("<i", payload, 36)[0]
    h_acc_mm = struct.unpack_from("<I", payload, 40)[0]
    v_acc_mm = struct.unpack_from("<I", payload, 44)[0]

    vel_n_mmps = struct.unpack_from("<i", payload, 48)[0]
    vel_e_mmps = struct.unpack_from("<i", payload, 52)[0]
    vel_d_mmps = struct.unpack_from("<i", payload, 56)[0]
    gspeed_mmps = struct.unpack_from("<i", payload, 60)[0]
    head_mot_1e5 = struct.unpack_from("<i", payload, 64)[0]

    gnss_fix_ok = bool(flags & 0x01)

    return {
        "iTOW": iTOW,
        "datetime": (year, month, day, hour, minute, sec),
        "fix_type": fix_type,
        "gnss_fix_ok": gnss_fix_ok,
        "num_sv": num_sv,
        "lat_deg": lat,
        "lon_deg": lon,
        "alt_m": height_msl_mm / 1000.0,
        "h_acc_m": h_acc_mm / 1000.0,
        "v_acc_m": v_acc_mm / 1000.0,
        "vel_n_mps": vel_n_mmps / 1000.0,
        "vel_e_mps": vel_e_mmps / 1000.0,
        "vel_d_mps": vel_d_mmps / 1000.0,
        "ground_speed_mps": gspeed_mmps / 1000.0,
        "head_mot_deg": head_mot_1e5 * 1e-5,
    }


class MaxM10sI2CNode(Node):
    def __init__(self):
        super().__init__("gnss_maxm10s_i2c_node")

        self.declare_parameter("i2c_bus", 1)
        self.declare_parameter("i2c_addr", 0x42)
        self.declare_parameter("poll_hz", 2.0)
        self.declare_parameter("max_read_bytes", 256)
        self.declare_parameter("publish_vel", True)

        self.i2c_bus_num = int(self.get_parameter("i2c_bus").value)
        self.addr = int(self.get_parameter("i2c_addr").value)
        self.poll_hz = float(self.get_parameter("poll_hz").value)
        self.max_read = int(self.get_parameter("max_read_bytes").value)
        self.publish_vel = bool(self.get_parameter("publish_vel").value)

        self.pub_fix = self.create_publisher(NavSatFix, "/gps/fix", 10)
        self.pub_vel = self.create_publisher(TwistStamped, "/gps/vel", 10)
        self.pub_diag = self.create_publisher(DiagnosticArray, "/gps/diagnostics", 10)

        self.parser = UbxStreamParser()
        self.last_pvt_time = None
        self.last_diag_pub = 0.0
        self.debug_printed_proto = False

        self.bus = None
        self._connect_bus()

        period = 1.0 / max(self.poll_hz, 1e-3)
        self.timer = self.create_timer(period, self.poll_once)

        self.get_logger().info(
            f"MAX-M10S I2C GNSS started on /dev/i2c-{self.i2c_bus_num}, addr 0x{self.addr:02X}"
        )

    def _close_bus(self):
        if self.bus is not None:
            try:
                self.bus.close()
            except Exception:
                pass
        self.bus = None
        #parser may hold partial UBX bytes from before the disconnect — drop them
        self.parser = UbxStreamParser()

    def _connect_bus(self) -> bool:
        self._close_bus()
        try:
            self.bus = SMBus(self.i2c_bus_num)
            time.sleep(0.2)
            self.configure_ubx()
            time.sleep(0.5)
            return True
        except Exception as e:
            self.get_logger().warn(f"GNSS I2C open failed: {e}")
            self._close_bus()
            return False

    def destroy_node(self):
        self._close_bus()
        super().destroy_node()

    def send_ubx(self, cls_id: int, msg_id: int, payload: bytes = b""):
        length = len(payload)

        header = bytes([
            cls_id,
            msg_id,
            length & 0xFF,
            (length >> 8) & 0xFF
        ])

        ck_a, ck_b = ubx_checksum(header + payload)

        msg = bytes([0xB5, 0x62]) + header + payload + bytes([ck_a, ck_b])

        #proper I2C block write
        self.bus.write_i2c_block_data(self.addr, msg[0], list(msg[1:]))

        self.get_logger().info("UBX config message sent")

    def configure_ubx(self):
        self.get_logger().info("Configuring GNSS for UBX output")

        try:
            #disable common NMEA messages on DDC/I2C; payload = [cls, id, rateDDC, rateUART1, rateUART2, rateUSB, rateSPI, reserved]

            #GGA
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x00, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #GLL
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x01, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #GSA
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x02, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #GSV
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x03, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #RMC
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x04, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #VTG
            self.send_ubx(0x06, 0x01, bytes([0xF0, 0x05, 0, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            #enable UBX NAV-PVT on DDC/I2C
            self.send_ubx(0x06, 0x01, bytes([0x01, 0x07, 1, 0, 0, 0, 0, 0]))
            time.sleep(0.05)

            self.get_logger().info("GNSS UBX configuration commands sent")
        except Exception as e:
            self.get_logger().warn(f"GNSS configuration error: {e}")

    def _read_avail(self) -> int:
        hi = self.bus.read_byte_data(self.addr, REG_AVAIL_H)
        lo = self.bus.read_byte_data(self.addr, REG_AVAIL_L)
        return (hi << 8) | lo

    def _read_stream(self, nbytes: int) -> bytes:
        n = min(nbytes, self.max_read)
        data = bytearray()

        for _ in range(n):
            data.append(self.bus.read_byte_data(self.addr, REG_STREAM))

        return bytes(data)

    def poll_once(self):
        if self.bus is None:
            if not self._connect_bus():
                self._publish_diag_if_needed(avail=0)
                return

        try:
            avail = self._read_avail()

            if avail == 0:
                self._publish_diag_if_needed(avail=0)
                return

            remaining = avail
            while remaining > 0:
                chunk = self._read_stream(remaining)
                if not chunk:
                    break

                if not self.debug_printed_proto:
                    self.debug_printed_proto = True

                remaining -= len(chunk)
                self.parser.feed(chunk)

                for cls_, id_, payload in self.parser.messages():
                    if cls_ == UBX_CLASS_NAV and id_ == UBX_ID_NAV_PVT:
                        pvt = parse_nav_pvt(payload)
                        if pvt:
                            self._publish_pvt(pvt)

            self._publish_diag_if_needed(avail=avail)

        except OSError as e:
            self.get_logger().warn(f"I2C read error, will reconnect: {e}")
            self._close_bus()
        except Exception as e:
            self.get_logger().warn(f"GNSS node exception, will reconnect: {e}")
            self._close_bus()

    def _publish_pvt(self, pvt: dict):
        now = self.get_clock().now().to_msg()
        self.last_pvt_time = time.time()

        msg = NavSatFix()
        msg.header.stamp = now
        msg.header.frame_id = "gps_link"

        msg.status.service = NavSatStatus.SERVICE_GPS
        if pvt["gnss_fix_ok"] and pvt["fix_type"] >= 3:
            msg.status.status = NavSatStatus.STATUS_FIX
        else:
            msg.status.status = NavSatStatus.STATUS_NO_FIX

        msg.latitude = float(pvt["lat_deg"])
        msg.longitude = float(pvt["lon_deg"])
        msg.altitude = float(pvt["alt_m"])

        h = max(pvt["h_acc_m"], 0.0)
        v = max(pvt["v_acc_m"], 0.0)
        msg.position_covariance = [
            h * h, 0.0, 0.0,
            0.0, h * h, 0.0,
            0.0, 0.0, v * v
        ]
        msg.position_covariance_type = NavSatFix.COVARIANCE_TYPE_DIAGONAL_KNOWN

        self.pub_fix.publish(msg)

        if self.publish_vel:
            vel = TwistStamped()
            vel.header.stamp = now
            vel.header.frame_id = "gps_link"
            vel.twist.linear.x = float(pvt["vel_e_mps"])
            vel.twist.linear.y = float(pvt["vel_n_mps"])
            vel.twist.linear.z = float(-pvt["vel_d_mps"])
            self.pub_vel.publish(vel)

    def _publish_diag_if_needed(self, avail: int):
        t = time.time()
        if (t - self.last_diag_pub) < 1.0:
            return
        self.last_diag_pub = t

        age = (t - self.last_pvt_time) if self.last_pvt_time else float("inf")

        st = DiagnosticStatus()
        st.level = DiagnosticStatus.OK if age < 2.0 else DiagnosticStatus.WARN
        st.name = "gnss_maxm10s_i2c"
        st.message = "OK" if age < 2.0 else "No recent NAV-PVT"
        st.values = [
            KeyValue(key="i2c_addr", value=f"0x{self.addr:02X}"),
            KeyValue(key="avail_bytes_last_poll", value=str(avail)),
            KeyValue(key="age_s_since_last_pvt", value=f"{age:.2f}"),
        ]

        arr = DiagnosticArray()
        arr.header.stamp = self.get_clock().now().to_msg()
        arr.status = [st]
        self.pub_diag.publish(arr)


def main():
    rclpy.init()
    node = MaxM10sI2CNode()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()


if __name__ == "__main__":
    main()