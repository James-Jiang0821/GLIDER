#!/usr/bin/env python3
"""Mission state machine driving controller and Iridium lifecycle nodes through IDLE/INITIALISE/OPERATION/EMERGENCY phases."""
...
class StateManagerNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Float64Stamped, "/pressure/depth", self._depth_cb, 10)
    ...
    def _depth_cb(self, msg: Float64Stamped):
    ...
    def _ctrl_lc_call(self, transition_id: int) -> Optional[bool]:
        """Async lifecycle transition for the controller node."""
    ...
    def home_result_callback(self, future):
        ...
        self.get_logger().info(
            f"home_result_callback: status={status} "
            f"(SUCCEEDED={GoalStatus.STATUS_SUCCEEDED}), "
    ...
    def _cancel_home_goal_if_pending(self):
        ...
        if handle is None:
            return
        self.get_logger().warn(
            "EMERGENCY: cancelling in-flight /bridge/home_actuators goal")
        try:
            handle.cancel_goal_async()
        except Exception as e:
            self.get_logger().warn(f"cancel_goal_async raised: {e}")
        self._home_goal_handle = None
    ...
    def step(self):
        ...
        if self.emergency_triggered and self.state != MissionState.EMERGENCY:
            ...
            self._cancel_home_goal_if_pending()
            self.emergency_phase = EmergencyPhase.CHECKING_DEPTH
            self.transition_to(MissionState.EMERGENCY)
